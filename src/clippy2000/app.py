...
class App(QMainWindow):
    ...
    def refresh_middle_layout(self):
        ...
        for i in range(len(self.current_taskqueue)):
            ...
            task_selector.addItems(self.taskQueue.available_tasks.keys())
            ...
            if self.taskQueue.available_tasks[self.current_taskqueue[i].split(':')[0]]['arguments'] == 0:
                optional_vars[0].setDisabled(True)
                optional_vars[1].setDisabled(True)  
                #block out both
            elif self.taskQueue.available_tasks[self.current_taskqueue[i].split(':')[0]]['arguments'] == 1:
                #block out 1
                optional_vars[1].setDisabled(True)
    ...
    def vars_edited(self, var_index, i, text):
        ...
        if self.taskQueue.available_tasks[self.current_taskqueue[i].split(':')[0]]['arguments'] == 1:
            self.current_taskqueue[i] = self.current_taskqueue[i].split(':')[0] + ':' + text
        #can have 2 arguments and this is the first one
        elif var_index == 0 and self.taskQueue.available_tasks[self.current_taskqueue[i].split(':')[0]]['arguments'] == 2:
    ...
    def task_edited(self, i, task):
        ...
        if task == "ADD NEW":
            ...
            task = task + self.taskQueue.available_tasks[task]['arguments']*':'#this adds a task with the correct number of possible colons
