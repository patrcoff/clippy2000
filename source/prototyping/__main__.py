...
class App:
    ...
    def process_queue(
        ...
        pyperclip.copy(
            run_queue(parser(self.commands[queue_ref]), in_text, debug=self.debug)
        )
    ...
    def show_window(self, icon, item):  # the main GUI window is configured here
        ...
        clicked.set(coms[0])
        label = Label(
            self.win, text=clicked.get()
        )  # the view below the dropdown and button

        def refresh_list(selected):
            label.config(text=self.commands[selected])

        commands_list = OptionMenu(
            self.win, clicked, *coms, command=refresh_list
        ...
        def add_option():
            USER_INP = simpledialog.askstring(
                title="New command queue:",
                prompt="Enter unique name for command queue:",
            )
            print(USER_INP)
    ...
    def hide_window(self):
        ...
        submenu = Menu(
            lambda: (item(x, partial(self.process_queue, x)) for x in self.commands)
        )
        self.win.withdraw()
        image = Image.open("favicon.ico")
        menu = (
            item("Editor", self.show_window),
    ...
    def run_app(self):
        ...
        self.win.protocol(
            "WM_DELETE_WINDOW", partial(self.hide_window)
